...
options:
  ...
  sites:
    ...
    suboptions:
      ...
      bandwidth_limit: {description: Default per-bucket bandwidth limit in bytes per second., type: int}
...
def run(module, client):
    ...
    if module.params["state"] == "absent":
        ...
        changed = bool(existing) if module.params["remove_all"] else bool(set(names) & set(existing))
        if changed and not module.check_mode:
            client.remove_site_replication(
                sites=",".join(names) if names else None, all_sites=module.params["remove_all"]
            )
        module.exit_json(changed=changed, site_replication=info)
